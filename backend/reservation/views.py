...
class AddFacilityView(APIView):
    def post(self, request):
        ...
        if Facility.objects.filter(facility_name=facility_name).exists():
            resp = {
                "status": "error",
                "message": f"A facility with the name '{facility_name}' already exists.",
            }
            return JsonResponse(
                resp,
    ...
    def delete(self, request):
        ...
        try:
            ...
            if Court.objects.filter(facility=facility).exists():
                resp = {
                    "status": "error",
                    "message": "Cannot delete facility",
...
class AddCourtView(APIView):
    def post(self, request):
        ...
        if not court_name or not facility_name:
            resp = {
                "status": "error",
                "message": "Both court_name and facility_name are required.",
            }
            return JsonResponse(
                resp,
                status=status.HTTP_400_BAD_REQUEST,
        ...
        try:
            ...
            resp = {
                "status": "error",
                "message": f"Facility '{facility_name}' does not exist.",
            }
            return JsonResponse(
                resp,
                status=status.HTTP_404_NOT_FOUND,
    ...
    def delete(self, request):
        ...
        try:
            ...
            if ReservationRequest.objects.filter(
                court=court, date__gte=datetime.now()
            ).exists():
                resp = {
                    "status": "error",
                    "message": "Cannot delete court",
    ...
    def post(self, request):
        ...
        for date_entry in dates:
            ...
            # day rate
                ...
                duration_label = (
                    "per full day" if duration_type == "full_day" else "per half day"
                )
                court_rate = CourtRate.objects.filter(
                    court=court,
                    activity=reservation_data["activity"],
            ...
            return Response(
                {
                    ...
                    "details": reservation_request.errors,
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        for date_entry in dates:
            ReservationDate.objects.create(
                ...
                start_time=(
                    date_entry.get("start_time") if rate_type == "hourly_rate" else None
                ),
                end_time=(
                    date_entry.get("end_time") if rate_type == "hourly_rate" else None
                ),
                duration_type=(
                    date_entry.get("duration_type") if rate_type == "day_rate" else None
                ),
            )
            ...
            response_data = [
                {
                    ...
                    "reservation": {
                        "facility": (
                            rd.reservation.court.facility.facility_name
                            if rd.reservation
                            else None
                        ),
                        "court": (
                            rd.reservation.court.court_name if rd.reservation else None
                        ),
